...
def main():
    ...
    send_email(
        "[CHTC Job Update]", "Training job started on file %s @ time %s" %
        (args.filename, datetime.datetime.now()))
    try:
        train(args.filename)
        send_email(
            "[CHTC Job Update]",
            "Ending job successfully on file %s @ time %s" %
            (args.filename, datetime.datetime.now()))
        ...
        send_email(
            ...
            (str(e), args.filename, datetime.datetime.now(), stack_trace))
...
def dataloader(filename, batch_size=16):
    ...
    for img, mask in zip(x_test, y_test):
        data["test"].append((torch.tensor(img).type(torch.FloatTensor),
